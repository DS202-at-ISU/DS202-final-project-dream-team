...
## Part I: Project report {.unnumbered}
...
THINGS TO NOTE - page number on slide - facet by categorical variables - not too much text on slides - he's kinda picky on what type of figure you use - stacked vs dodge bar chart - percent vs count bar chart
...
-   The primary variables that will be used will be category, which includes the type of good and/or service; reported_year, which is the year the consumer data was reported; and value, which contains the amount of money spent for a specific category and year.
...
Looking at these box plots, it's clear why service spending is higher than goods spending. Health care and housing/utilities dominate consumer spending overall. Then, the next items that consumers spend the most on are grocery items, included in miscellaneous nondurable goods and also food and beverages.
...
goods_data <- clean_data %>%
  ...
  mutate(type = case_when(
    ...
    str_detect(category, "Goods") ~ "Nondurable",
    TRUE ~ NA_character_
  )) %>%
  filter(!is.na(type)) 
...
ggplot(clean_data, aes(x = value_millions)) + 
  ...
  geom_vline(aes(xintercept = mean(value_millions)), color = "red", linetype = "dashed", size = 1) +
  geom_vline(aes(xintercept = median(value_millions)), color = "blue", linetype = "dashed", size = 1) + 
...
ggplot(clean_data, aes(x = date, y = value, color = expenditure_type)) + 
  geom_smooth(method = "loess", se = FALSE, aes(color = expenditure_type), size = 1.2) +  # Smoothed line only
