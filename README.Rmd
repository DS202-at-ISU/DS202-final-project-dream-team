...
This model is a part of my honors component for this course decided early in the semester with the instructor Will Ju. The honors component required me to create some kind of a model to predict consumer spending in Iowa and I chose a random forest as it allows us to have multiple factors to predict aspects in each category. I noticed that the model did not perfect too well and I suspecr it is due to the presence of multiple cumulative categories. As suggested in the presentation, future research could create a random forest model for each category to acheive better results.
